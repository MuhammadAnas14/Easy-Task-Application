  ...
  const tracklocation = () => {
    ...
    if (item.taskMethod == "live" && item.status == "0.25") {
      ...
      for (let k = 0; k < BidsIterator.length; k++) {
        if (BidsIterator[0].UserId === item.taskAssignTo) {
  ...
  return (
    <View style={styles.listItem}>
      ...
      <View style={styles.bugget}>
        ...
        >
          <Text
            style={{ color: "white", backgroundColor: "#3CAABB", padding: 5 }}
          >
            Open
          </Text>
        </TouchableOpacity>
        <Text style={{ color: "#3CAABB", paddingRight: 5 }}>
          Rs.{item.taskBudget}
        </Text>
      </View>
    </View>
